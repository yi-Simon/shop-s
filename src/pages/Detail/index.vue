            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  :key="attrValue.id"
                  @click="changeAttrValue(attr.spuSaleAttrValueList,index)"
...
export default {
  ...
  methods: {
    ...
    changeAttrValue(target, index) {
      target.forEach((item) => {
        item.isChecked = "0";
      });
      target[index].isChecked = "1";
    },
    plus() {
      ++this.skuNum;
    },
    ...
    async addToCart() {
      try {
        ...
        if (result.status === 200) {
          alert("添加成功");
          sessionStorage.setItem("SKUID_KEY", JSON.stringify(this.skuInfo));
          this.$router.push({ name: "addToCart", query: { skuNum } });
        }
      } catch (e) {
        alert(e.message);
      }
